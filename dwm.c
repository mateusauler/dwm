...
resizeclient(Client *c, int x, int y, int w, int h)
{
	...
	if (c->isfloating || c->mon->lt[c->mon->sellt]->arrange == NULL) {
		...
		if (smarthidegaps && (c->mon->lt[c->mon->sellt]->arrange == monocle || n == 1)) {
			gapoffset = 0;
			gapincr = -2 * borderpx;
			wc.border_width = 0;
		} else {
...
updatesystray(void)
{
	...
	if (!systray) {
		...
		if (!(systray = (Systray *)calloc(1, sizeof(Systray))))
			die("fatal: could not malloc() %u bytes\n", sizeof(Systray));
		systray->win = XCreateSimpleWindow(dpy, root, x, by, w, bh, 0, 0, scheme[SchemeSel][ColBg].pixel);
	...
	for (w = 0, i = systray->icons; i; i = i->next) {
		...
		w += systrayspacing;
		i->x = w;
		XMoveResizeWindow(dpy, i->win, i->x, user_bh ? (user_bh - drw->fonts->h) / 2 : 1 , i->w, i->h);
		w += i->w;
		if (i->mon != m)
			i->mon = m;
	}
	...
	x -= w;
	XMoveResizeWindow(dpy, systray->win, x, by, w, bh);
	wc.x = x; wc.y = by; wc.width = w; wc.height = bh;
